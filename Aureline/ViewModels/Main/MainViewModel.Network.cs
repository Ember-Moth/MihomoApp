...
public partial class MainViewModel
{
    ...
    private async Task RefreshNetworkDetectionAsync()
    {
        ...
        try
        {
            ...
            using var cancellation = new CancellationTokenSource(
                IsRunning ? TimeSpan.FromSeconds(20) : TimeSpan.FromSeconds(10));
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            var (client, shouldDisposeClient) = CreateNetworkDetectionClient();
            try
            {
    ...
    private (HttpClient Client, bool ShouldDispose) CreateNetworkDetectionClient()
    {
        if (!IsRunning)
        {
        ...
        {
            UseProxy = true,
            Proxy = new WebProxy($"http://127.0.0.1:{CurrentMixedPort()}")
        };
        return (new HttpClient(handler, disposeHandler: true), true);
    }
